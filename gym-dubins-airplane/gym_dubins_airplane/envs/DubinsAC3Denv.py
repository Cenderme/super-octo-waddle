...
class DubinsAC3Denv(gym.Env):
    ...
    def step(self, action):
        ...
        self._blueAC.takeaction(cmd_bank_deg, cmd_theta_deg, self._vel_mps,
                                self._action_time_s)
    ...
    def _random_pos(self):
        pos0 = np.array(
            [self.window_width / 4, self.window_height / 4, self.window_z / 4])

        return (pos0 +
                np.random.uniform(low=np.array([0, 0, 0]),
                                  high=np.array([
                                      self.window_width / 2,
                                      self.window_height / 2, self.window_z / 2
