...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size,
                 experiences_per_sampling, seed, compute_weights):
        """Initialize a ReplayBuffer object.
    ...
    def update_priorities(self, tds, indices):
        for td, index in zip(tds, indices):
            ...
            if self.compute_weights:
                updated_weight = (
                    (N * updated_priority)**(-self.beta)) / self.weights_max
            ...
            data = self.data(updated_priority, updated_probability,
                             updated_weight, index)
    ...
    def update_parameters(self):
        ...
        for element in self.memory_data.values():
            ...
            if self.compute_weights:
                weight = (
                    (N * element.probability)**(-self.beta)) / self.weights_max
    ...
    def add(self, state, action, reward, next_state, done):
        ...
        if self.experience_count > self.buffer_size:
            ...
            if temp.priority == self.priorities_max:
                ...
                self.priorities_max = max(self.memory_data.items(),
                                          key=operator.itemgetter(1)).priority
            if self.compute_weights:
                if temp.weight == self.weights_max:
                    self.memory_data[index].weight = 0
                    self.weights_max = max(self.memory_data.items(),
                                           key=operator.itemgetter(2)).weight
