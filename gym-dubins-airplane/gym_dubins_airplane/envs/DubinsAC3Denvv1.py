...
class DubinsAC3Denvv1(gym.Env):
    ...
    def step(self, action):
        ...
        self._blueAC.takeaction(cmd_bank_deg, cmd_theta_deg, cmd_vel_mps,
                                self._action_time_s)
    ...
    def reset(self):
        ...
        pos[1] += 200
        pos[2] = -pos[2] - 200
        self._redAC = ACEnvironment2D(position=np.array(
            [pos[0], pos[1], pos[2]]),
        ...
        self._blueAC = ACEnvironment2D(position=np.array(
    ...
    def _random_pos(self):
        pos0 = np.array(
            [self.area_width / 4, self.area_height / 4, self.area_z / 4])

        return (pos0 + np.random.uniform(
            low=np.array([0, 0, 0]),
            high=np.array(
                [self.area_width / 2, self.area_height / 2, self.area_z / 2])),
                np.random.uniform(low=-180, high=180))

    def _random_pos2(self):
        return (np.random.uniform(low=np.array([0, 0, 0]),
                                  high=np.array([
                                      self.area_width * 0.5,
                                      self.area_height * 0.5, self.area_z * 0.5
                                  ])), np.random.uniform(low=-180, high=180))
    ...
    def _get_sta_env_v2(self):
        ...
        self.errPos, self.errDist, _, _ = self._calc_posDiff_hdg3d_rad(
            Bpos, self.goal_pos)
        _, _, self.LOSxy_deg, self.LOSz_deg = self._calc_posDiff_hdg3d_rad(
            Bpos, Rpos)

        self.ATAxy_deg = np.rad2deg(
            self._pi_bound(self.LOSxy_deg - Batt_rad[2]))
        ...
        _, self.targetDist, self.redATAxy_deg, self.redATAz_deg = self._calc_posDiff_hdg3d_rad(
            Rpos, Bpos)
        self.redATAxy_deg = np.rad2deg(
            self._pi_bound(self.redATAxy_deg - Ratt_rad[2]))
        self.redATAz_deg = np.rad2deg(
            self._pi_bound(self.redATAz_deg - Ratt_rad[1]))

        return np.array([
            self.errPos[0] / self.area_width,
    ...
    def _get_sta_env_v2_redAC(self):
        ...
        _, targetDist, redATAxy_deg, redATAz_deg = self._calc_posDiff_hdg3d_rad(
            Bpos, Rpos)
        ...
        return np.array([
            errPos[0], errPos[1], errPos[2], LOSxy_deg, LOSz_deg, ATAxy_deg,
            ATAz_deg, AA_deg, redATAxy_deg, redATAz_deg,
