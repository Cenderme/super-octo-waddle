...
class DubinsAC2Denv(gym.Env):
    ...
    def step(self, action): # by using input action, environment returns an observation and reward_sca
	...
	# action takes values from 0 to 14
        ...
        envSta = self._get_sta_env_v2() # envSta list is what will be fed into Q-network
    ...
    def render(self, mode='human'):
        ...
        blue_ac_img._color.vec4 = (1, 1, 1, 1) # siyah renk hatası giderildi / 15.02 
        jtransform = rendering.Transform(rotation= -att[2], translation=np.array([pos[1], pos[0]]))
    ...
    def _get_sta_env_v2(self): # returns state of blue aircraft which is the input of DQN
        ...
        r = R.from_euler('zyx', [self.Ratt_rad[2], self.Ratt_rad[1], self.Ratt_rad[0]])
        ...
        self.ATA_deg = np.rad2deg(self._pi_bound(self.LOS_deg - self.Batt_rad[2]))
        self.AA_deg = np.rad2deg(self._pi_bound(self.Ratt_rad[2] - self.LOS_deg))
        ...
        self.redATA_deg = np.rad2deg(self._pi_bound(self.redATA_deg - self.Ratt_rad[2]))

        return np.array([self.errPos[0], # removed redATA_deg from feature vector 
                         self.errPos[1],
                         self.LOS_deg,
                         self.ATA_deg,
                         self.AA_deg,
    ...
    def _get_sta_env_v2_redAC(self): # return state of red aircraft
        ...
        target_dist = np.array([0., 0., 0.])
        r = R.from_euler('zyx', [self.Batt_rad[2], self.Batt_rad[1], self.Batt_rad[0]])
        target_dist = np.matmul(r.as_matrix(), target_dist)
    ...
    def scalar_reward(self): # Method for calculating scalar reward
        ...
        self.distance_ = math.sqrt((self.Bpos[0]-self.Rpos[0])**2 + (self.Bpos[1]-self.Rpos[1])**2) # distance between two aircrafts
        TERMINALSTATE = False
        if Sca_RewOpt:
          if ((-30 < self.ATA_deg < 30) and (-15 < self.AA_deg < 15) and (self.d_min < self.distance_ < self.d_max)): # dominant area, blue win
              if random.random() < 0.8: # chance to hit enemy in dominant area
                ...
                DAMAGE_redAC = 0
                print(' \n[MISS] Blue! Reward: {}\n'.format(reward_sca))
          elif ((180 > self.ATA_deg > 120) and (180 > self.AA_deg > 150) and (self.d_min < self.distance_ < self.d_max)): # lose 
              reward_sca = -3
              ...
              reward_sca = -2
              print(' Aircrafts crashed into each other! Reward: {}'.format(reward_sca))
              TERMINALSTATE = True
          else:
              TERMINALSTATE = False

        return reward_sca, TERMINALSTATE, DAMAGE_redAC, {'result': INFO,
                                       'redObs': self._get_sta_env_v2_redAC()}
                                       
    def shaping_reward(self): # Method for calcualting potential based shaping reward
      ...
      if Pot_RewOpt:
        ...
        D = np.exp((-abs(self.distance_-(self.d_max + self.d_min)/2))/(180*k))
